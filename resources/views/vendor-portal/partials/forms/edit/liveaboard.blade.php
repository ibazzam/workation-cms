{{-- Standalone edit form: Liveaboard / Safari --}}
<form class="ops-form" method="POST" action="/portal/vendor/properties/{{ $propertyId }}/update">
    ...
    <div class="ops-form-grid">
        ...
        <div class="ops-field ops-field-wide">
            ...
            <input id="property_name" name="name" class="ops-input" type="text" maxlength="160" value="{{ old('name', $property->name ?? '') }}" placeholder="e.g. Male' to Seenu Gan Safari" required>
        </div>
        <div class="ops-field ops-field-wide">
        ...
        <div class="ops-field">
            ...
            <input id="property_start_point" name="start_point" class="ops-input" type="text" maxlength="120" value="{{ old('start_point', $propertyDetails['start_point'] ?? '') }}" placeholder="e.g. Malé" required>
        </div>
        <div class="ops-field">
            <label for="property_end_point">End Point (Disembark) <span style="color:#c0392b;">*</span></label>
            <input id="property_end_point" name="end_point" class="ops-input" type="text" maxlength="120" value="{{ old('end_point', $propertyDetails['end_point'] ?? '') }}" placeholder="e.g. Seenu Gan" required>
        </div>
        <div class="ops-field">
            <label for="property_journey_duration_days">Journey Duration (days) <span style="color:#c0392b;">*</span></label>
            <input id="property_journey_duration_days" name="journey_duration_days" class="ops-input" type="number" min="1" max="90" value="{{ old('journey_duration_days', $propertyDetails['journey_duration_days'] ?? '') }}" required>
        </div>
        ...
        <p style="grid-column:1/-1; font-size:0.9rem; color:#666; margin-bottom:1rem;">Add stopovers where guests can board or disembark along the journey. Enter each on a new line in format: <strong>StopoverName|AllowEmbark|AllowDisembark</strong> (e.g., "Laamu Kahdhoo|yes|yes" or "Addu City|no|yes").</p>
        <div class="ops-field ops-field-wide">
            <label for="property_stopovers">Stopovers List</label>
            <textarea id="property_stopovers" name="stopovers" class="ops-textarea" rows="4" maxlength="1000" placeholder="Laamu Kahdhoo|yes|yes&#10;Gan Island|yes|yes&#10;Addu City|no|yes">{{ old('stopovers', implode("\n", (array) ($propertyDetails['stopovers'] ?? []))) }}</textarea>
        ...
        <p style="grid-column:1/-1; font-size:0.9rem; color:#666; margin-bottom:1rem;">Define package prices for different boarding-disembarking combinations. Format: <strong>FromPoint→ToPoint=Price(MVR)</strong>. Set one per line.</p>
        <div class="ops-field ops-field-wide">
            <label for="property_pricing_matrix">Pricing Matrix</label>
            <textarea id="property_pricing_matrix" name="pricing_matrix" class="ops-textarea" rows="5" maxlength="2000" placeholder="Malé→Seenu Gan=5000&#10;Malé→Gan=4500&#10;Laamu→Seenu Gan=3500">{{ old('pricing_matrix', implode("\n", (array) ($propertyDetails['pricing_matrix'] ?? []))) }}</textarea>
        ...
        <div class="ops-field">
            ...
            <input id="property_vessel_name" name="vessel_name" class="ops-input" type="text" maxlength="120" value="{{ old('vessel_name', $propertyDetails['vessel_name'] ?? '') }}" placeholder="e.g. Island Safari">
        </div>
        <div class="ops-field">
        ...
        <div class="ops-field">
            ...
            <input id="property_max_guests" name="max_guests" class="ops-input" type="number" min="1" max="1000" value="{{ old('max_guests', $property->max_guests ?? '') }}">
        </div>
        <div class="ops-field">
            <label for="property_cabin_count">Cabin / Room Count</label>
            <input id="property_cabin_count" name="cabin_count" class="ops-input" type="number" min="1" value="{{ old('cabin_count', $propertyDetails['cabin_count'] ?? '') }}">
        </div>
        ...
        <div class="ops-field ops-field-wide">
            ...
            <textarea id="property_boarding_instructions" name="boarding_instructions" class="ops-textarea" rows="3" maxlength="1500" placeholder="Arrive 2 hours before departure. Bring your booking confirmation and ID.">{{ old('boarding_instructions', $propertyDetails['boarding_instructions'] ?? '') }}</textarea>
        </div>

...
class HttpTransportProviderAdapter
{
    ...
    protected function callProvider(string $provider, string $path, array $payload = []): array
    {
        ...
        try {
            ...
            $requestId = $payload['request_id'] ?? ($payload['meta']['request_id'] ?? (string) Str::uuid());
            ...
            if ($resp->successful()) {
                ...
                return ['ok' => true, 'body' => $resp->json(), 'retryable' => false, 'request_id' => $requestId];
            }
            ...
            return ['ok' => false, 'error' => 'provider error: ' . $resp->status(), 'body' => $resp->body(), 'retryable' => $retryable, 'request_id' => $requestId];
        } catch (\Throwable $e) {
            $msg = $e->getMessage();
            ...
            return ['ok' => false, 'error' => $msg, 'retryable' => true, 'request_id' => $requestId ?? null];
        }
    }

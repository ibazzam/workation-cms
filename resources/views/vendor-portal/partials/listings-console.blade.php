                                        ...
                                        <tbody>
                                            @foreach ($categoryProperties as $property)
                                                @php
                                                    ...
                                                    $isCorporateRetreatPackage = in_array($categoryKey, ['excursion', 'water_sports'], true)
                                                        && (
                                                            (int) ($property->is_corporate_retreat ?? 0) === 1
                                                            || (int) ($property->is_retreat_package ?? 0) === 1
                                                            || in_array(strtolower(trim((string) ($propertyDetails['is_corporate_retreat'] ?? '0'))), ['1', 'true', 'yes', 'on'], true)
                                                            || in_array(strtolower(trim((string) ($propertyDetails['is_retreat_package'] ?? '0'))), ['1', 'true', 'yes', 'on'], true)
                                                        );
                                                    $retreatInclusionsText = trim((string) ($propertyDetails['inclusions'] ?? ''));
                                                    $retreatInclusionItems = collect(preg_split('/[\r\n,;]+/', $retreatInclusionsText ?: ''))
                                                        ->map(static fn ($item) => trim((string) $item))
                                                        ->filter(static fn ($item) => $item !== '')
                                                        ->unique(static fn ($item) => strtolower($item))
                                                        ->take(8)
                                                        ->values()
                                                        ->all();
                                                            ...
                                                            <div class="listing-card-main">
                                                                <div class="listing-summary-line">
                                                                    ...
                                                                    @if ($isCorporateRetreatPackage)
                                                                        <span class="ops-chip" title="Corporate Retreat Package">Corporate Retreat</span>
                                                                    @endif
                                                                ...
                                                                @if ($isCorporateRetreatPackage)
                                                                    <div style="margin-top:8px;display:flex;flex-wrap:wrap;gap:6px;">
                                                                        @php
                                                                            $resolveRetreatIcon = static function (string $item): string {
                                                                                $normalized = strtolower(trim($item));
                                                                                if (str_contains($normalized, 'room') || str_contains($normalized, 'villa') || str_contains($normalized, 'stay')) {
                                                                                    return 'fa-solid fa-bed';
                                                                                }
                                                                                if (str_contains($normalized, 'transport') || str_contains($normalized, 'transfer') || str_contains($normalized, 'boat') || str_contains($normalized, 'airport')) {
                                                                                    return 'fa-solid fa-van-shuttle';
                                                                                }
                                                                                if (str_contains($normalized, 'meal') || str_contains($normalized, 'breakfast') || str_contains($normalized, 'lunch') || str_contains($normalized, 'dinner') || str_contains($normalized, 'food')) {
                                                                                    return 'fa-solid fa-utensils';
                                                                                }
                                                                                if (str_contains($normalized, 'spa') || str_contains($normalized, 'wellness')) {
                                                                                    return 'fa-solid fa-spa';
                                                                                }
                                                                                if (str_contains($normalized, 'activity') || str_contains($normalized, 'excursion') || str_contains($normalized, 'tour')) {
                                                                        ...
                                                                        @endphp
                                                                        @foreach ($retreatInclusionItems as $retreatInclusionItem)
                                                                            <span class="ops-chip" style="display:inline-flex;align-items:center;gap:6px;"><i class="{{ $resolveRetreatIcon((string) $retreatInclusionItem) }}" aria-hidden="true"></i>{{ $retreatInclusionItem }}</span>
                                                                        @endforeach
                                                                        @if (empty($retreatInclusionItems))
                                                                            <span class="ops-chip">Set package inclusions (Room, Transport, Meals, etc.)</span>
                                                                        @endif
                                                                    </div>
                                                                @endif
                                                            </div>
                                                            ...
                                                            <tr class="accommodation-room-stretch-row">
                                                                <td colspan="2">
                                                                    <div class="accommodation-room-stretch">
                                                                        <p class="property-subsection-head">Room Categories ({{ $propertyRooms->count() }})</p>

            ...
            <tbody>
                @forelse ($newsletters as $newsletter)
                    <tr>
                        ...
                        <td>{{ strtoupper((string) ($newsletter->status ?? 'draft')) }}</td>
                        <td>{{ strtoupper((string) ($newsletter->audience ?? 'all')) }}</td>
                        <td>{{ optional($newsletter->scheduled_at)->format('Y-m-d H:i') ?? '—' }}</td>
                        <td>{{ optional($newsletter->updated_at)->format('Y-m-d H:i') ?? '—' }}</td>
                        <td>
                            <a class="btn" href="{{ '/portal/admin/newsletter/' . $newsletter->id . '/edit' }}">Edit</a>
                            @if (strtoupper((string) session('portal_admin_role', '')) === 'ADMIN_SUPER')
                                <form method="POST" action="{{ '/portal/admin/newsletter/' . $newsletter->id . '/delete' }}" style="display:inline;">
                                    @csrf
                                    <button class="btn" type="submit" onclick="return confirm('Delete this newsletter?');">Delete</button>
                                </form>
                            @endif
                        </td>
                    </tr>
                @empty
                    <tr>
                        <td colspan="7">No newsletters yet.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>

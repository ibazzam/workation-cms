...
<head>
    ...
    <title>{{ (string) ($property->name ?? 'Vehicle Rental') }} | Workation</title>
...
<body>
@php
    $headerCategoryKey = 'vehicle-rental';
    $headerCategoryLinks = [
    ...
    $amenitiesRaw = [];
    foreach (['amenities', 'features', 'services', 'highlights'] as $key) {
...
<main class="page">
    <nav class="breadcrumb">
        ...
        <a href="/catalog/vehicle_rental">Vehicle Rentals</a>
        <span aria-hidden="true">›</span>
        <span>{{ (string) ($property->name ?? 'Vehicle Rental') }}</span>
    </nav>

    <section class="section" aria-label="Gallery">
        <h2>Photo Gallery</h2>
        <div class="gallery-shell" data-gallery>
            <img id="galleryHero" class="gallery-hero" src="{{ $gallery->first() ?: $imageUrl }}" alt="Vehicle image" loading="lazy" onerror="if(!this.src.startsWith('data:')){this.onerror=null;this.src='{{ $imageUrl }}';}">
    ...
    <section class="property-summary-shell" aria-label="Summary">
        <div class="property-summary-main">
            <h1>{{ (string) ($property->name ?? 'Vehicle Rental') }}</h1>
            <div class="property-summary-meta">
                <span><i class="fa-solid fa-car-side" aria-hidden="true"></i> Vehicle rental available</span>
            </div>
            @if ($description !== '')
        ...
        <aside class="property-summary-price">
            ...
            <span class="sub">Per day rental</span>
            <a class="booking-btn" href="/category-booking/vehicle-rental/{{ $property->vendor_property_id ?? $property->id }}"><i class="fa-solid fa-calendar-check" aria-hidden="true"></i> Book Vehicle</a>
        </aside>
    </section>
    ...
    <section id="amenities-section" class="section">
        ...
        @else
            <div class="empty-note">Vehicle amenities will be published shortly.</div>
        @endif
    </section>
    ...
    <section id="policies-section" class="section">
        ...
        <div class="policies-grid">
            <article class="policy-card">
                <div class="policy-label">Booking Terms</div>
                <div class="policy-value">{{ trim((string) ($listingDetails['booking_policy'] ?? 'Minimum 24-hour advance booking required.')) }}</div>
            </article>
            <article class="policy-card">
                <div class="policy-label">Cancellation</div>
                <div class="policy-value">{{ trim((string) ($listingDetails['cancellation_policy'] ?? 'Free cancellation up to 48 hours before rental start.')) }}</div>
            </article>
            <article class="policy-card">
                <div class="policy-label">Insurance & Fuel</div>
                <div class="policy-value">{{ trim((string) ($listingDetails['fuel_policy'] ?? 'Insurance included. Vehicle returns with full tank.')) }}</div>
            </article>
            <article class="policy-card">
                <div class="policy-label">Requirements</div>
                <div class="policy-value">{{ trim((string) ($listingDetails['requirement_policy'] ?? 'Valid international driving license and passport required.')) }}</div>
            </article>
        </div>
    </section>
</main>

...
<form class="ops-form" method="POST" action="/portal/vendor/properties/create">
    ...
    <div class="ops-form-grid">
        ...
        <div class="ops-field">
            <label for="property_departure_point">Departure Point</label>
            <input id="property_departure_point" name="departure_point" class="ops-input" type="text" maxlength="120" value="{{ old('departure_point') }}" placeholder="e.g. Malé Harbour" required>
        </div>
        <div class="ops-field">
            <label for="property_arrival_point">Arrival Point</label>
            <input id="property_arrival_point" name="arrival_point" class="ops-input" type="text" maxlength="120" value="{{ old('arrival_point') }}" placeholder="e.g. Seenu Gan" required>
        </div>
        ...
        <div class="ops-field">
            <label for="property_return_time">Return / Arrival Time</label>
            <input id="property_return_time" name="return_time" class="ops-input" type="time" value="{{ old('return_time') }}" required>
        </div>
        <div class="ops-field">
            <label for="property_trip_duration_minutes">Trip Duration (minutes)</label>
            <input id="property_trip_duration_minutes" name="trip_duration_minutes" class="ops-input" type="number" min="1" value="{{ old('trip_duration_minutes') }}" required>
        </div>
        ...
        {{-- Availability Slots (similar to excursion departure_slots pattern) --}}
        <div class="ops-field ops-field-wide" style="border-top: 1px solid #e0e0e0; padding-top: 1rem; margin-top: 1rem;">
            <label style="font-weight: 600; display: block; margin-bottom: 0.5rem;">Availability</label>
            <p style="font-size: 0.9rem; color: #666; margin-bottom: 1rem;">Enter available days and dates when this route operates (one per line). Format: Mon,Wed,Fri or specific dates YYYY-MM-DD</p>
        </div>
        <div class="ops-field ops-field-wide">
            <label for="property_availability_schedule">Operating Schedule</label>
            <textarea id="property_availability_schedule" name="availability_schedule" class="ops-textarea" rows="3" maxlength="1000" placeholder="Mon&#10;Wed&#10;Fri&#10;2026-05-15">{{ old('availability_schedule') }}</textarea>
        </div>

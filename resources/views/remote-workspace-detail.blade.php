...
@php
    ...
    $internetSpeed = trim((string) ($listingDetails['internet_speed'] ?? $listingDetails['wifi_speed'] ?? ''));
    $seatingCapacity = (int) ($listingDetails['seating_capacity'] ?? $listingDetails['capacity'] ?? 0);
    $availabilityHours = trim((string) ($listingDetails['availability_hours'] ?? $listingDetails['working_hours'] ?? ''));
    ...
    $amenitiesRaw = [];
    foreach (['amenities', 'features', 'services', 'highlights', 'facilities'] as $key) {
...
<main class="page">
    <nav class="breadcrumb">
        ...
        <span aria-hidden="true">›</span>
        <span>{{ (string) ($property->name ?? 'Remote Workspace') }}</span>
    </nav>

    <section class="section" aria-label="Gallery">
        <h2>Photo Gallery</h2>
        <div class="gallery-shell" data-gallery>
            <img id="galleryHero" class="gallery-hero" src="{{ $gallery->first() ?: $imageUrl }}" alt="Workspace image" loading="lazy" onerror="if(!this.src.startsWith('data:')){this.onerror=null;this.src='{{ $imageUrl }}';}">
    ...
    <section class="property-summary-shell" aria-label="Summary">
        <div class="property-summary-main">
            <h1>{{ (string) ($property->name ?? 'Remote Workspace') }}</h1>
            <div class="property-summary-meta">
                <span><i class="fa-solid fa-laptop" aria-hidden="true"></i> Professional workspace</span>
                @if ($internetSpeed !== '')
                    <span><i class="fa-solid fa-wifi" aria-hidden="true"></i> {{ $internetSpeed }}</span>
                @endif
                @if ($seatingCapacity > 0)
                    <span><i class="fa-solid fa-users" aria-hidden="true"></i> Capacity: {{ $seatingCapacity }}</span>
                @endif
        ...
        <aside class="property-summary-price">
            ...
            <a class="booking-btn" href="/category-booking/remote_workspace/{{ $property->vendor_property_id ?? $property->id }}"><i class="fa-solid fa-calendar-check" aria-hidden="true"></i> Book Workspace</a>
        </aside>
    </section>
    ...
    <section id="amenities-section" class="section">
        ...
        @else
            <div class="empty-note">Workspace amenities will be published shortly.</div>
        @endif
    </section>
    ...
    <section id="policies-section" class="section">
        ...
        <div class="policies-grid">
            <article class="policy-card">
                ...
                <div class="policy-value">{{ trim((string) ($listingDetails['booking_policy'] ?? 'Book your workspace in advance to secure preferred slots.')) }}</div>
            </article>
            <article class="policy-card">
                <div class="policy-label">Cancellation</div>
                <div class="policy-value">{{ trim((string) ($listingDetails['cancellation_policy'] ?? 'Free cancellation up to 24 hours before start time.')) }}</div>
            </article>
            <article class="policy-card">
                <div class="policy-label">Access & Check-in</div>
                <div class="policy-value">{{ trim((string) ($listingDetails['access_policy'] ?? 'Present booking confirmation and valid ID at reception.')) }}</div>
            </article>
            <article class="policy-card">
                <div class="policy-label">Usage Rules</div>
                <div class="policy-value">{{ trim((string) ($listingDetails['house_rules'] ?? 'Please maintain a professional environment and keep shared spaces clean.')) }}</div>
            </article>
        </div>
    </section>
</main>
